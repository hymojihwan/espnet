...
forced_alignments= # CTC based forced alignment.
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
        ...
        for dset in ${_dsets}; do
            ...
            # Copy forced_alignments.npz if it exists
            if [ -f "data/${dset}/forced_alignments.npz" ]; then
                log "Copying forced_alignments.npz for ${dset} to ${data_feats}${_suf}/${dset}"
                cp "data/${dset}/forced_alignments.npz" "${data_feats}${_suf}/${dset}/forced_alignments.npz"
                cp "data/${dset}/forced_alignments.npz" "${data_feats}/${dset}/forced_alignments.npz"
            else
                log "Warning: forced_alignments.npz not found for ${dset}"
            fi
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ] && ! [[ " ${skip_stages} " =~ [[:space:]]10[[:space:]] ]]; then
    ...
    # 0. Add forced alignments
    _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/forced_alignments.npz,forced_alignments,alignment "
