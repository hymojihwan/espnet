...
enh_tag="enh_train_enh_se_jepa_frontend_raw"
...
# 1) JEPA frontend(exp) + pretrained ASR(no-spec)
./run_se_jepa_frontend.sh \
    ...
    --inference_asr_tag rnnt_nospec_pretrained_jepaenh \
    2>&1 | tee exp/${enh_tag}/run_pretrained_jepa_nospec_stage910.log

# 2) JEPA frontend(exp) + pretrained ASR(spec)
./run_se_jepa_frontend.sh \
    --stage 9 --stop_stage 10 \
    --valid_set "" \
    --enh_tag "${enh_tag#enh_}" \
    ...
    --inference_asr_model valid.loss.ave_10best.pth \
    --inference_asr_config conf/tuning/transducer/decode_transducer.yaml \
    --inference_asr_tag rnnt_spec_pretrained_jepaenh \
    2>&1 | tee exp/${enh_tag}/run_pretrained_jepa_spec_stage910.log
