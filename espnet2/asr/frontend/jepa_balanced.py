...
class JEPA_BalancedFrontend(AbsFrontend):
    """JEPA Balanced Frontend for ASR.

    This frontend implements a JEPA-style self-supervised learning approach
    with clean reconstruction loss computed on ALL patches (not just masked).
    ...
    Architecture:
        ...
        Loss: L_align = MSE(pred_latents[masked], target_latents[masked])
              L_clean_recon = MSE(clean_patches_hat, clean_patches) [ALL patches]
    """
    ...
    def __init__(
        ...
        self._last_decoded_patches = None  # Decoded patches from noisy path (for unmasked loss)
        self._last_noisy_patches = None  # Noisy patches (for denoising loss)
    ...
    def forward(
        ...
        if mask.any() and self.training and clean_input is not None:
            ...
            # Decode ALL patches (both masked and unmasked) for loss calculation
            # Decode all predicted latents to patches
            all_decoded_patches = self.decoder(predicted_latents.view(-1, self.embedding_dim))  # (B*num_patches, patch_dim)
            all_decoded_patches = all_decoded_patches.view(B, num_patches, -1)  # (B, num_patches, patch_dim)
            
            # For final output: only replace masked patches (stable inference)
            # Unmasked patches remain as noisy input
            full_reconstructed_patches = noisy_patches.clone()
            full_reconstructed_patches[mask] = all_decoded_patches[mask]  # Replace only masked patches
            ...
            self._last_decoded_patches = all_decoded_patches  # Store all decoded patches for unmasked loss
            self._last_noisy_patches = noisy_patches  # Store noisy patches for denoising loss
        else:
            # Inference mode: predict and decode masked patches only (stable output)
            if mask.any():
                if self.predictor_type == "transformer":
                    # Transformer predictor: process all context features
                    pred_features = context_features  # (B, num_patches, embedding_dim)
                ...
                # Merge patches back into spectrogram: only replace masked patches
                full_reconstructed_patches = noisy_patches.clone()
    ...
    def compute_jepa_loss(self) -> Optional[torch.Tensor]:
        ...
        2. L_clean_recon = MSE(clean_patches_hat, clean_patches) [ALL valid patches]
        3. L_unmask = Denoising loss on unmasked patches
           - Encourages decoder to denoise unmasked patches from noisy to clean
           - Loss = MSE(decoded[unmasked], clean[unmasked]) + penalty if decoded is worse than noisy
        
        Note: Unlike jepa_audio.py, clean_recon loss is computed on ALL valid patches,
        not just masked patches. Additionally, unmasked loss is a denoising loss that
        encourages the decoder to improve unmasked patches from noisy to clean.
        """
        if (
            ...
            or self._last_decoded_patches is None
        ):
            return None
        ...
        unmasked_valid = (~self._last_mask) & self._last_patch_mask  # (B,N)
        ...
        # Use ALL valid patches (not just masked)
        if self._last_patch_mask.any():
            clean_hat_all = self._last_clean_patches_hat[self._last_patch_mask]  # (M_all, patch_dim)
            clean_all = self._last_clean_patches[self._last_patch_mask]  # (M_all, patch_dim)
            loss_clean_recon = F.mse_loss(clean_hat_all, clean_all, reduction="mean")
        else:
            loss_clean_recon = torch.tensor(0.0, device=self._last_clean_patches_hat.device)
        
        # 3. Unmasked denoising loss: L_unmask
        # This encourages the decoder to denoise unmasked patches from noisy to clean
        # The loss ensures that decoded output is closer to clean than the noisy input
        if unmasked_valid.any() and self.unmask_loss_weight > 0:
            decoded_unmasked = self._last_decoded_patches[unmasked_valid]  # (M_unmask, patch_dim) - denoised output
            noisy_unmasked = self._last_noisy_patches[unmasked_valid]  # (M_unmask, patch_dim) - noisy input
            clean_unmasked = self._last_clean_patches[unmasked_valid]  # (M_unmask, patch_dim) - clean target
            
            # Primary denoising loss: decoded should be close to clean
            loss_denoise = F.mse_loss(decoded_unmasked, clean_unmasked, reduction="mean")
            
            # Improvement constraint: decoded should be better than noisy
            # Calculate per-patch MSE, then check if decoded is worse than noisy
            mse_decoded_clean = F.mse_loss(decoded_unmasked, clean_unmasked, reduction="none").mean(dim=-1)  # (M,)
            mse_noisy_clean = F.mse_loss(noisy_unmasked, clean_unmasked, reduction="none").mean(dim=-1)  # (M,)
            # Penalty when decoded is worse than noisy (positive difference means decoded is worse)
            improvement_penalty = (mse_decoded_clean - mse_noisy_clean).clamp(min=0.0).mean()
            
            # Combine: denoising loss + improvement penalty
            # The improvement penalty ensures the decoder actually denoises, not just copies noisy
            loss_unmask = loss_denoise + 0.1 * improvement_penalty
        else:
            loss_unmask = torch.tensor(0.0, device=self._last_clean_patches.device)
        
        total_loss = (
            loss_align * self.embedding_loss_weight
            + loss_clean_recon * self.embedding_loss_weight
