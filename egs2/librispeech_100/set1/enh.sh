...
[ -z "${valid_set}" ] && [ ${stop_stage} -lt 9 ] && { log "${help_message}"; log "Error: --valid_set is required"  ; exit 2; };
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        # Optional text prior conditioning
        if [ -f "${_enh_train_dir}/token_int" ]; then
            _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/token_int,token_int,text_int "
            _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/token_int_shape "
        fi
        if [ -f "${_enh_train_dir}/token_time_ms" ]; then
            _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/token_time_ms,token_time_ms,text_float "
            _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/token_time_ms_shape "
        fi
        if [ -f "${_enh_train_dir}/token_chunk_int" ]; then
            _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/token_chunk_int,token_chunk_int,text_int "
            _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/token_chunk_int_shape "
        fi
        if [ -f "${_enh_valid_dir}/token_int" ]; then
            _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/token_int,token_int,text_int "
            _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/token_int_shape "
        fi
        if [ -f "${_enh_valid_dir}/token_time_ms" ]; then
            _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/token_time_ms,token_time_ms,text_float "
            _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/token_time_ms_shape "
        fi
        if [ -f "${_enh_valid_dir}/token_chunk_int" ]; then
            _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/token_chunk_int,token_chunk_int,text_int "
            _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/token_chunk_int_shape "
        fi
...
if "${score_with_asr}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        _cmd=${decode_cmd}
        # This recipe evaluates RNN-T ASR checkpoints.
        _infer_module=espnet2.bin.asr_transducer_inference
