#!/usr/bin/env bash
# Joint Conv-TasNet frontend + scratch Conformer-CTC ASR for CHiME4.
#
# Enhancement:
#   - load pretrained Conv-TasNet from ../enh1
#   - initialize from pretrained enhancement weights and fine-tune jointly
#
# ASR:
#   - train from scratch
#   - char tokenization
#   - CTC-only decoding/scoring
#
# Stages:
#   0: prepare enh_asr1 data
#   1: collect stats
#   2: train
#   3: decode
#   4: score
...
case "${enh_model_size}" in
    small)
        ...
        if [ "${token_type}" = bpe ]; then
            config=conf/tuning/train_enh_s2t_joint_enh_conformer_hybrid_small.yaml
        else
            config=conf/tuning/train_enh_s2t_joint_enh_conformer_ctc_small.yaml
        fi
        stats_dir="exp/enh_s2t_joint_enh_ctc_stats_${token_tag}_small"
        expdir="exp/enh_s2t_joint_enh_ctc_conformer_${token_tag}_small"
        joint_freeze_enh_modules="${JOINT_FREEZE_ENH_MODULES:-}"
        train_fold_length=40000
        ;;
    base)
        enh1_exp_dir="${_script_dir}/../enh1/exp/enh_train_enh_conv_tasnet_raw"
        config=conf/tuning/train_enh_s2t_joint_enh_conformer_ctc.yaml
        stats_dir="exp/enh_s2t_joint_enh_ctc_stats_${token_tag}"
        expdir="exp/enh_s2t_joint_enh_ctc_conformer_${token_tag}"
        joint_freeze_enh_modules="${JOINT_FREEZE_ENH_MODULES:-enh_model.encoder enh_model.separator}"
        train_fold_length=20000
        ;;
    *)
        log "ERROR: unsupported ENH_MODEL_SIZE=${enh_model_size} (expected: base or small)"
        exit 1
        ;;
esac
...
if [ "${token_type}" = bpe ]; then
    ...
    inference_tag="decode_asr_${lm_weight_tag}_joint_model_valid.acc.ave_10best"
    ...
    inference_tag="decode_ctc_bs1_joint_model_valid.cer_ctc.ave_10best"
...
prepare_speed_perturb() {
    ...
    if [ ! -f "${format_wav_scp}" ]; then
        log "ERROR: format_wav_scp.sh not found: ${format_wav_scp}"
        exit 1
    fi
    ...
    if [ ! -f "data/${base_train_set}_sp/wav.scp" ] || [ ! -f "data/${base_train_set}_sp/text_spk1" ]; then
        ...
        if [ ! -f "${perturb_script}" ]; then
            log "ERROR: perturb script not found: ${perturb_script}"
            exit 1
        fi
...
if [ ! -f "${TOKEN_LIST}" ]; then
    log "ERROR: token list not found: ${TOKEN_LIST}"
    exit 1
fi
...
if [ "${stage}" -le 1 ] && [ "${stop_stage}" -ge 1 ]; then
    ...
    ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
        ...
        || {
            cat "$(ls -t "${_logdir}"/stats.*.log 2>/dev/null | head -1)" 2>/dev/null || true
            exit 1
        }
...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    for _module in ${joint_freeze_enh_modules}; do
        [ -n "${_module}" ] || continue
        _train_args+=(--freeze_param "${_module}")
    done
...
if [ "${stage}" -le 3 ] && [ "${stop_stage}" -ge 3 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        log "Decoding ${dset} ... log: ${_logdir}/asr_inference.*.log"
        ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
            ...
            || { cat $(grep -l -i error "${_logdir}"/asr_inference.*.log) ; exit 1; }
...
if [ "${stage}" -le 4 ] && [ "${stop_stage}" -ge 4 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        for _type in wer cer; do
            ...
            if [ "${_type}" = wer ]; then
                paste \
                    <(<"${_data}/text_spk1" ${python} -m espnet2.bin.tokenize_text \
                        -f 2- --input - --output - \
                        --token_type word \
                        --non_linguistic_symbols data/nlsyms.txt \
                ...
                paste \
                    <(<"${_dir}/text_spk1" ${python} -m espnet2.bin.tokenize_text \
                        -f 2- --input - --output - \
                        --token_type word \
                        --non_linguistic_symbols data/nlsyms.txt \
                        --remove_non_linguistic_symbols true) \
    ...
    bash scripts/utils/show_asr_result.sh "${expdir}" > "${expdir}/RESULTS.md" || true
    log "Wrote ${expdir}/RESULTS.md"
fi
