...
feats_normalize=global_mvn # Normalizaton layer type. Use global_mvn (stats will be computed from JEPA embeddings)
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
        ...
        for dset in ${_dsets}; do
            ...
            # Copy clean_wav.scp if it exists (for JEPA training)
            if [ -f "data/${dset}/clean_wav.scp" ]; then
                cp "data/${dset}/clean_wav.scp" "${data_feats}${_suf}/${dset}/clean_wav.scp"
            fi
            ...
            # Handle clean_wav.scp if it exists (for JEPA training)
            # If clean_wav.scp points to the same files as wav.scp, use the same ark files
            if [ -f "data/${dset}/clean_wav.scp" ]; then
                # Create a temporary clean_wav.scp with utterance IDs that match wav.scp
                # We'll create clean_wav.scp by matching utterance IDs from wav.scp
                python3 -c "
import sys
wav_scp = {}
with open('${data_feats}${_suf}/${dset}/wav.scp', 'r') as f:
    for line in f:
...
clean_wav_scp = {}
with open('data/${dset}/clean_wav.scp', 'r') as f:
    for line in f:
        parts = line.strip().split(None, 1)
        if len(parts) == 2:
            clean_wav_scp[parts[0]] = parts[1]

# For each utterance in clean_wav.scp, use the ark file from wav.scp if it exists
with open('${data_feats}${_suf}/${dset}/clean_wav.scp', 'w') as f:
    for utt_id, _ in clean_wav_scp.items():
        if utt_id in wav_scp:
            f.write(f'{utt_id} {wav_scp[utt_id]}\n')
        else:
            # If utterance doesn't exist in wav.scp, we need to process it separately
            # But this shouldn't happen in normal JEPA setup
            sys.stderr.write(f'Warning: {utt_id} not found in wav.scp\n')
" || exit 1
            fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ] && ! [[ " ${skip_stages} " =~ [[:space:]]4[[:space:]] ]]; then
    ...
    for dset in "${train_set}" "${valid_set}"; do
        ...
        # Copy clean_wav.scp if it exists (for JEPA training)
        if [ -f "${data_feats}/org/${dset}/clean_wav.scp" ]; then
            cp "${data_feats}/org/${dset}/clean_wav.scp" "${data_feats}/${dset}/clean_wav.scp"
        fi
        ...
        if [ "${_feats_type}" = raw ]; then
            ...
            # Filter clean_wav.scp if it exists (for JEPA training)
            if [ -f "${data_feats}/org/${dset}/clean_wav.scp" ]; then
                <"${data_feats}/org/${dset}/clean_wav.scp" \
                    utils/filter_scp.pl "${data_feats}/${dset}/utt2num_samples"  \
                    >"${data_feats}/${dset}/clean_wav.scp"
            fi
        else
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ] && ! [[ " ${skip_stages} " =~ [[:space:]]10[[:space:]] ]]; then
    ...
    if [ "${_feats_type}" = raw ]; then
        ...
        # Convert fs from "16k" format to integer (e.g., "16k" -> 16000)
        _fs_value="${fs}"
        if [[ "${_fs_value}" == *k ]]; then
            _fs_value="${_fs_value%k}000"
        fi
        _opts+="--frontend_conf fs=${_fs_value} "
    else
        _scp=feats.scp
        _type=kaldi_ark
    ...
    # Add clean_wav.scp if it exists (for JEPA training)
    if [ -f "${_asr_train_dir}/clean_wav.scp" ]; then
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/clean_wav.scp,clean_speech,${_type} "
    fi
    if [ -f "${_asr_valid_dir}/clean_wav.scp" ]; then
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/clean_wav.scp,clean_speech,${_type} "
    fi
    # shellcheck disable=SC2068
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    if [ "${num_splits_asr}" -gt 1 ]; then
        ...
        # Add clean_wav.scp if it exists (for JEPA training)
        if [ -f "${_asr_train_dir}/clean_wav.scp" ]; then
            # Split clean_wav.scp as well
            if [ ! -f "${_split_dir}/clean_wav.scp.done" ]; then
                ${python} -m espnet2.bin.split_scps \
                  --scps \
                      "${_asr_train_dir}/clean_wav.scp" \
                  --num_splits "${num_splits_asr}" \
                  --output_dir "${_split_dir}"
                touch "${_split_dir}/clean_wav.scp.done"
            fi
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/clean_wav.scp,clean_speech,${_type} "
        fi
        # shellcheck disable=SC2068
    ...
    # Add clean_wav.scp for validation if it exists (for JEPA training)
    if [ -f "${_asr_valid_dir}/clean_wav.scp" ]; then
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/clean_wav.scp,clean_speech,${_type} "
    fi
    # shellcheck disable=SC2068
