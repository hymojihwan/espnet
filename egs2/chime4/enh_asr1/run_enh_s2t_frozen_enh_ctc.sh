#!/usr/bin/env bash
# Frozen Conv-TasNet frontend + scratch Conformer-CTC ASR for CHiME4.
#
# Enhancement:
#   - load pretrained Conv-TasNet from ../enh1
#   - freeze the whole enhancement frontend
#
# ASR:
#   - train from scratch
#   - char tokenization
...
case "${enh_model_size}" in
    small)
        ...
        if [ "${token_type}" = bpe ]; then
            config=conf/tuning/train_enh_s2t_frozen_enh_conformer_hybrid_small.yaml
        else
            config=conf/tuning/train_enh_s2t_frozen_enh_conformer_ctc_small.yaml
        fi
        stats_dir="exp/enh_s2t_frozen_enh_ctc_stats_${token_tag}_small"
        expdir="exp/enh_s2t_frozen_enh_ctc_conformer_${token_tag}_small"
        ;;
    base)
        ...
        stats_dir="exp/enh_s2t_frozen_enh_ctc_stats_${token_tag}"
        expdir="exp/enh_s2t_frozen_enh_ctc_conformer_${token_tag}"
        ;;
    *)
...
if [ "${token_type}" = bpe ]; then
    ...
    inference_tag="decode_asr_${lm_weight_tag}_asr_model_valid.acc.ave_10best"
    ...
    inference_tag="decode_ctc_bs1_asr_model_valid.cer_ctc.ave_10best"
